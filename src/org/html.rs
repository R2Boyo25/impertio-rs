        ...
        for section in doc.sections {
            for node in section.nodes {
                match node {
                    ...
                    Node::LesserBlock { type_, args, contents } => {
                        match type_.as_str() {
                            "src" => {
                                if args.len() > 0 {
                                    self.builder.add_preformatted(format!("<code class=\"hljs {}\">{}</code>", args[0], contents.replace("\n", "<br/>")));
                                } else {
                                    self.builder.add_preformatted(format!("<code>{}</code>", contents.replace("\n", "<br/>")));
                                }
                            },
                            _ => {
                                todo!();
...
mod test {
    ...
    fn paragraphs() {
        assert_eq!(
            HtmlBuilder::new().from_document(Document::parse(r#"Hello,
  world!
Hewwo!
