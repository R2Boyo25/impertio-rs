...
impl FileHandler {
    ...
    fn handle_file(&mut self, data_dir: PathBuf, root: PathBuf, rel_file: PathBuf) {
        ...
        match file.extension().unwrap_or(&OsStr::new("")).to_str().unwrap_or("") {
            "org" => {
                ...
                if !file_changed(&file, &new_file).unwrap() && !file_changed(&file, &source_file).unwrap() {
                    return;
                }
                ...
                let out = self.templates.render("root.html", file.as_path(), &parsed.to_html(), Some(parsed.metadata.iter().map(|(key, value)| (key.as_str(), value.to_owned())).collect())).unwrap();

                log::info!("{}: {}", file.to_str().unwrap(), out);

                writeable(&new_file).unwrap().write_all(out.as_bytes()).unwrap();
                writeable(&source_file).unwrap().write_all(std::fs::read(file).unwrap().as_slice()).unwrap();
            },
            "html" => (),
            _ => {
                ...
                writeable(&new_file).unwrap().write_all(std::fs::read(file).unwrap().as_slice()).unwrap();
            }
        }
    }
    ...
    pub fn handle_files(&mut self, data_dir: String, dir: String) {
        ...
        for file in walkdir::WalkDir::new(dir.clone())
            ...
            .filter(filter_file) {
            self.handle_file(data_path.clone(), root_path.clone(), path_to_rel_path(root_path.clone(), file))
        }
    }
}
